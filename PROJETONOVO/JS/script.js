...
$("input[class='obrigatorio']").blur(function () {
  if (!$(this).val()) {
    ...
    $(this).css("border", "5px solid red").focus()
    console.log(this.name, "vazio")
  } else {
    listaInputs++
    $(this).css("border", "none")
    console.log(this.name, "preenchido")
  }
})

$("#btnEnviar").click(function () {
  if (listaInputs < 6) {
    caixaModal("Atenção!", "Preencha os campos obrigatórios")
  } else {
    caixaModal("Obrigado por se cadastrar!", "Formulário enviado.")
...
function consultaCep() {
  ...
  if (cepDigitado.value == "") {
    ...
    fetch(`http://viacep.com.br/ws/${cepProcurado}/json`)
      ...
      .catch(x => caixaModal("Atenção!", "CEP não encontrado"))
  }
}
